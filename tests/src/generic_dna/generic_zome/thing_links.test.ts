...
test("Create a bidirectional link between to Things and verify correctness of the link tag contents", async () => {
  await runScenario(async (scenario) => {
    ...
    assert.equal(
      ...
      encodeHashToBase64(thing1.id)
    );
    assert(!!linkedThingIds[0][1].backlink_action_hash);
    assert.equal(linkedThingIds[0][1].thing_created_at, thing1.created_at);
    assert.equal(
      encodeHashToBase64(linkedThingIds[0][1].thing_created_by),
      encodeHashToBase64(thing1.creator)
    );
    assert.deepEqual(linkedThingIds[0][1].target_node_id, {
      type: "Thing",
      id: thing1.id,
    });
    ...
    assert.equal(
      encodeHashToBase64(linkedThingIds2[0][0]),
      encodeHashToBase64(thing2.id)
    );
    assert(!linkedThingIds2[0][1].backlink_action_hash);
    assert.equal(linkedThingIds2[0][1].thing_created_at, thing2.created_at);
    assert.equal(
      encodeHashToBase64(linkedThingIds2[0][1].thing_created_by),
      encodeHashToBase64(thing2.creator)
    );
    assert.deepEqual(linkedThingIds2[0][1].target_node_id, {
      type: "Thing",
      id: thing2.id,
    });
